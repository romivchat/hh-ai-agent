...
В `candidate_profile.json` хранятся варианты позиционирования, компетенции и только проверенные публичные формулировки достижений. Добавьте название текущего работодателя и его продукта в `forbidden_terms`: тогда они не попадут в письмо. Файл исключён из Git, должен иметь права `600` и не должен публиковаться.
...
- `Дополнить данные` показывает формулировку перед сохранением и пересобирает письмо только после подтверждения;
